<?php
$news_one_post = get_field($args['field_name'], 'option') ?: [];

if (!empty($news_one_post)) :
    $title = get_the_title($news_one_post);
    $permalink = get_permalink($news_one_post);
    $excerpt = wp_trim_words(get_the_excerpt($news_one_post), 40, '...');
    $thumbnail = get_the_post_thumbnail_url($news_one_post, 'full');
    $date = get_the_date('d F Y', $news_one_post);
    $categories = get_the_category($news_one_post);
